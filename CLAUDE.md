...
> Note: this is the **app/store-level (offline-style)** credential model — one credential resolved once at startup, shared by all callers — and it is **single-tenant**. This is the *current implementation's* choice, **not** a Shopify limitation: Shopify also supports **per-user OAuth "online access mode"** tokens bound to an individual signed-in staff user (carry `associated_user`, expire with the user's session, enforce that user's own permissions). Under Jarvis the **registry owns OAuth** and forwards the user's token + shop per request; the container stays a stateless resource server.
>
> **Chosen v1 auth direction (D26, June 2026):** because the server is read-only and admin-operated, v1 uses an **offline (app-level) authorization-code token** (long-lived, no ~24h session expiry — *not* per-user/online), obtained via Shopify's **unified-admin store picker** (`admin.shopify.com`) so the shop is chosen at sign-in — replicating the Claude connector's entry UX against **our own** app + raw Admin OAuth (not Shopify's first-party hosted-MCP `resource=setup.shopify.com/mcp`). **One shop per connection; switch stores by revoke/uninstall + re-auth.** The container is unchanged by this (`shopRouting.ts` already consumes the forwarded `X-Shopify-Shop-Domain`); the open work is registry-side (per-shop `token_url` templating from the callback `shop`, = O7). See `SHOPIFY_MCP_FORK_PLAN.md` §5 "Chosen bring-up variant (D26)" and decision **D26**.
